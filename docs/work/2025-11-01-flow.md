...
This is the only file that should be tracked by Git. It is a single, append-only file where each line is a JSON object representing one "event" (a change) in the system.
  ...
  * **Conflict-free:** Git merge conflicts are resolved by a custom merge driver that concatenates versions, deduplicates lines, preserves append order, and relies on ULID ordering during replay.
...
1.  User runs `beads sync`.
2.  `beads` opens `beads.db` and reads `last_processed_offset` and `last_event_id` from the `_meta` table (defaults `0` and `null`).
3.  It opens `events.jsonl`, calls `seek(last_processed_offset)`, and streams every subsequent line into memory.
4.  It parses the new events, sorts them by `event_id`, and validates that each ULID is strictly greater than the previously stored `last_event_id` (otherwise it triggers a recovery replay).
5.  It applies the sorted events as `UPDATE`s, `INSERT`s, or `DELETE`s to the *existing* `beads.db` using Last-Write-Wins semantics.
6.  It updates `_meta` with the byte offset immediately after the last processed event and persists the highest `event_id`.
7.  This process is nearly instantaneous.
...
3.  **Step 1 (Write to Log):** Appends this single line to `events.jsonl`, ensuring the ULID is persisted, and notes the starting byte offset returned by the file handle.
...
### 5.1. Git Merge Conflicts
  ...
  * **Solution: Custom Git Merge Driver.**
      ...
      * This keeps the merged `events.jsonl` append-only; on replay, `beads sync` loads new lines and sorts them by `event_id`, ensuring deterministic Last-Write-Wins resolution.
