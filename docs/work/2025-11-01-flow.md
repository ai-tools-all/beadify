Here is a detailed specification for the "Beads" issue tracker, built on the Event Sourcing model (Idea 3) and incorporating your requirements for fast incremental syncs and a human-friendly Markdown editing workflow.
...
The core philosophy is:
  ...
  * **Zero Merge Conflicts:** The event-sourcing model transforms all operations (create, update, link) into append-only writes, virtually eliminating Git merge conflicts.
  ...
  * **Offline & Fast:** Queries (`list`, `show`, `ready`) are always run against the local cache, making them instantaneous and available offline.
...
This is the only file that should be tracked by Git. It is a single, append-only file where each line is a JSON object representing one "event" (a change) in the system.
  ...
  * **Conflict-free:** Git merge conflicts are resolved by a custom merge driver that concatenates versions, deduplicates lines, and preserves append order.
...
This is the materialized view. The schema is identical to your original proposal, designed for fast queries.
  ...
  * **`_meta` table**: A private table for the sync mechanism.
      * `key` (TEXT): e.g., "last\_processed\_offset"
      * `value` (TEXT): e.g., "1234567" (stored as text but representing a byte offset)
...
4.  It iterates through the log in append order, building the final state of all issues in memory (a process called "reducing").
5.  It creates a new `beads.db` and performs bulk `INSERT`s to populate the `issues`, `dependencies`, `labels`, and `comments` tables.
6.  It records the byte offset immediately after the final event into the `_meta` table (`last_processed_offset`).
7.  This process may take 5-30 seconds, but is only done *once*.

### 4.2. Workflow: Fast Incremental Sync (Daily Pull)

**Goal:** A user pulls changes (`git pull`) and needs to update their local cache.

1.  User runs `beads sync`.
2.  `beads` opens `beads.db` and reads `last_processed_offset` from the `_meta` table (default `0`).
3.  It opens `events.jsonl`, calls `seek(last_processed_offset)`, and begins streaming every subsequent line in append order.
4.  It reads these 5, 10, or 100 new events without any reordering.
5.  It applies these events as `UPDATE`s, `INSERT`s, or `DELETE`s to the *existing* `beads.db`.
6.  It updates `_meta` with the byte offset immediately after the last processed event.
8.  This process is nearly instantaneous.

### 4.3. Workflow: Creating/Updating an Issue (CLI/Agent)

**Goal:** A user or AI agent closes an issue.

1.  User runs `beads update bd-42 --status closed --actor "alice"`.
2.  `beads` generates a new event object:
    `{"ts": "...", "op": "update", "id": "bd-42", "actor": "alice", "data": {"status": "closed"}}`
3.  **Step 1 (Write to Log):** Appends this single line to `events.jsonl` and notes the starting byte offset returned by the file handle.
4.  **Step 2 (Update Cache):** Applies this *same event* to the local `beads.db` (e.g., `UPDATE issues SET status = 'closed' WHERE id = 'bd-42'`).
5.  **Step 3 (Update Meta):** Updates `_meta(last_processed_offset)` to the byte position immediately after the appended event.
6.  The CLI returns "OK". The user's system is now 100% consistent.

### 4.4. Workflow: Human Spec-Editing (`beads spec edit`)

**Goal:** A human wants to edit the Markdown plan for an epic.

1.  User runs `beads spec edit bd-10`.
2.  **Step 1 (Read Cache):** `beads` queries the *local* `beads.db` for `bd-10`. (Fast).
3.  **Step 2 (Create Temp File):** It gets the `description`, `design`, `acceptance_criteria`, and `notes` fields and formats them into a temporary Markdown file:
    ```markdown
    # Issue: bd-10 (Fix login bug)
    #
    # !! DO NOT EDIT HEADERS !!
    ...
    (No notes yet)
    ```
4.  **Step 3 (Launch Editor):** `beads` launches the user's `$EDITOR` (e.g., Vim, VS Code) on this temp file and *waits*.
5.  **Step 4 (User Edits):** The user edits the file, saves, and closes the editor.
6.  **Step 5 (Parse Changes):** `beads` reads the *new* content from the temp file and parses it back into its constituent fields (description, design, etc.).
7.  **Step 6 (Write Event):** It generates a *new* `update` event and appends it to `events.jsonl`. The `data` payload contains *all four* text fields, ensuring the update is atomic.
    `{"ts": "...", "op": "update", "id": "bd-10", "actor": "...", "data": {"description": "...", "design": "...", ...}}`
8.  **Step 7 (Update Cache):** `beads` applies this new event to the local `beads.db` and advances `_meta(last_processed_offset)` to the new byte position.
9.  The CLI returns "OK".
...
### 5.1. Git Merge Conflicts
  ...
  * **Problem:** Two users append events to `events.jsonl` on different branches. `git merge` will combine the files, but the lines may be interleaved.
  * **Solution: Custom Git Merge Driver.**
      * We will provide a script and instruct users to set it up in `.gitattributes`:
        `*.jsonl merge=beadslog`
      * The `beadslog` merge driver (defined in `.git/config`) will be a small program that reads `$BASE`, `$LOCAL`, and `$REMOTE` in order, emits each line once (first occurrence wins), and preserves the original append order.
      * This keeps the merged `events.jsonl` append-only and ready for replay without any out-of-band reordering.
      * The `beads sync` command trusts on-disk append order and never re-sorts its input, guaranteeing deterministic reduction.
...
### 5.2. Log Compaction (`events.jsonl` getting too big)
  ...
  * **Solution: `beads compact` Command.**
      ...
      * This "squashes" history, making the log small and fast for all future clones.
...
## 6\. Updated CLI Command Reference
  ...
  * **Core Write Commands (Append Log):**
  ...
  * **Sync & Maintenance:**
      ...
      * `beads sync`: (Fast Incremental) Updates local `beads.db` from `events.jsonl`.
