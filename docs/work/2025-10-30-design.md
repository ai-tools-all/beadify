...
**Philosophy**:
- JSONL by default (zero setup, human-diffable, syncs cleanly via git)
- SQLite/PostgreSQL for teams/scale (multi-user, better performance)
...
graph TB
    ...
    subgraph "Current Implementation"
        JSONL[JSONL Storage<br/>✅ Implemented]
    end

    subgraph "Planned Future"
        SQLite[SQLite Storage<br/>📋 Planned Import/Export]
        Postgres[PostgreSQL Storage<br/>📋 Planned Phase 3]
    end

    subgraph "Data Layer"
        File[(JSONL File<br/>~/.beads/issues.jsonl)]
        DB2[(SQLite DB<br/>Generated on demand)]
        DB3[(PostgreSQL<br/>Not yet implemented)]
    end

    CLI --> Cobra
    Cobra --> Storage
    Storage --> JSONL
    Storage -.future.-> SQLite
    ...
    style JSONL fill:#4caf50,color:#fff
    style SQLite fill:#ffb74d,color:#fff
    style Postgres fill:#bdbdbd,color:#fff
    style File fill:#e8f5e9
    style DB2 fill:#fff3e0
    style DB3 fill:#f5f5f5
```
...
sequenceDiagram
    ...
    Storage->>JSONL: Append issue record
    Storage->>Events: Append event (created)
    JSONL-->>Storage: issue ID (bd-42)
    Storage-->>CLI: Success
    CLI-->>User: Created bd-42

    User->>CLI: bd dep add bd-42 bd-10
    CLI->>Storage: AddDependency(bd-42, bd-10)
    Storage->>JSONL: Update dependency set
    Storage->>Storage: Check for cycles in memory
    Storage->>Events: Append event (dep_added)
    Storage-->>CLI: Success
    CLI-->>User: Added dependency

    User->>CLI: bd ready
    CLI->>Storage: GetReadyWork()
    Storage->>JSONL: Load issues & dependencies
    JSONL-->>Storage: Issues with no blockers
...
2. **Import** - Run `bd import .beads/issues.jsonl` to materialize/update an optional SQLite cache
...
6. **Export** - Run `bd export ~/.beads/cache/beads.db .beads/issues.jsonl` (or project-specific paths) before committing
...
storage/
  ...
  jsonl/
    jsonl.go         // JSONL implementation (primary)
    encoder.go       // Buffered writer/reader utilities
  sqlite/
    sqlite.go        // Generated cache (optional, from JSONL)
    migrations.go    // Schema migrations (future)
  postgres/
...
```go
type Config struct {
    Backend string // "jsonl", "sqlite", or "postgres"

    // JSONL config
    JSONLPath string // default: ~/.beads/issues.jsonl

    // SQLite config (optional cache)
    SQLitePath string // default: ~/.beads/cache/beads.db
...
func NewStorage(config Config) (Storage, error) {
    ...
    case "jsonl":
        return jsonl.New(config.JSONLPath)
    case "sqlite":
        return sqlite.New(config.SQLitePath)
    case "postgres":
...
Each line in `.beads/issues.jsonl` is a standalone JSON document. Records are append-only with periodic compaction (rewrite) to preserve ordering and maintain deterministic diffs. Supported record kinds:

1. `issue` – canonical representation of a single issue (always includes full payload)
2. `dependency` – describes a `blocks`, `related`, or `parent-child` edge between two issues
3. `label` – associates a freeform label with an issue
4. `event` – audit trail entries (comments, status changes, metadata updates)
5. `meta` – global counters and checkpoints (e.g., next issue id)

Example slice:

```jsonl
{"type":"meta","key":"schema_version","value":1}
{"type":"meta","key":"next_issue_id","value":43}
{"type":"issue","id":"bd-42","title":"Fix login bug","description":"Users can't log in with Google OAuth","design":"","acceptance_criteria":"","notes":"","status":"open","priority":1,"issue_type":"bug","assignee":"alice","estimated_minutes":null,"created_at":"2025-10-30T22:14:03Z","updated_at":"2025-10-30T22:14:03Z","closed_at":null}
{"type":"dependency","issue_id":"bd-42","depends_on_id":"bd-10","dep_type":"blocks","created_at":"2025-10-30T22:16:40Z","created_by":"alice"}
{"type":"label","issue_id":"bd-42","label":"auth"}
{"type":"event","issue_id":"bd-42","event_type":"created","actor":"alice","old_value":null,"new_value":"{\"status\":\"open\"}","comment":null,"created_at":"2025-10-30T22:14:03Z"}
```

**Write Strategy**
- Mutations write to a temp file and atomically rename to avoid partial writes
- Compaction rewrites the entire file, merging the latest state per issue while preserving chronological events
- In-memory indexes (e.g., maps keyed by issue id) are built on startup for fast queries

**Ready Work Calculation**
- Loads issues + dependencies into memory, calculates ready set on demand
- Cache results in-memory; persist derived state as optional `meta` records if needed

### Future: SQLite Schema (Generated Cache)
...
Global Options:
  --store <path>       JSONL store path (default: ~/.beads/issues.jsonl)
  --backend <type>     Backend type: jsonl, sqlite, postgres (default: jsonl)
  --cache-db <path>    Optional SQLite cache location (default: ~/.beads/cache/beads.db)
...
```bash
# Initialize
beads init                           # Creates ~/.beads/issues.jsonl
beads init --store ./project.issues.jsonl   # Project-local JSONL store
beads init --backend postgres        # Interactive setup for PostgreSQL (bypasses JSONL)
...
#### JSONL ⇄ SQLite Bridges (Planned)

- `bd import myjson.jsonl` – materializes a writable SQLite cache (generated from the JSONL source)
- `bd export sqlite_db_path.db path/to/jsonfile.jsonl` – compacts or regenerates JSONL from an existing SQLite cache
```
...
- [ ] JSONL storage engine + compaction pipeline
...
- [ ] Deterministic JSONL formatter + diffs
- [ ] `bd import` / `bd export` JSONL workflows
...
- [ ] SQLite cache generation (optional) backed by JSONL import/export

### Phase 4: Advanced
- [ ] Git-integrated automations (pre-commit hooks, merge helpers)
...
1. **Zero setup**: Single file that ships with the repo
2. **Git-first**: Line-oriented format produces clean diffs and conflict resolution
3. **Human-friendly**: Inspect/edit with any text tooling
4. **Portability**: Works across platforms without drivers
5. **Interoperability**: Easy to transform into SQLite/PostgreSQL when needed
...
### Why Generate SQLite Caches?

1. **Performance**: Faster ad-hoc queries for large workspaces
2. **Tooling**: Easy to inspect with GUI browsers or sqlite CLI
3. **Compatibility**: Supports existing automation that expects relational data
4. **Bridge**: Enables round-tripping via `bd import` / `bd export`
...
Full audit trail lives in `event` records inside the JSONL stream. This enables:
- Change history for issues
- Comment threads
- "Who changed what when" debugging
- Potential undo/revert functionality (replay events)
...
encoding/json
bufio
github.com/google/uuid       // Issue ids (optional)

// Optional caches/backends
database/sql
github.com/mattn/go-sqlite3  // SQLite driver (cache/import/export)
...
1. **Multi-store support**: Should one beads installation manage multiple stores?
   - Probably yes: `beads --store project1/issues.jsonl` vs `beads --store project2/issues.jsonl`
...
6. **Concurrency**: How do we manage concurrent writers on a JSONL file?
   - Atomic rewrite strategy + file locks?
   - Provide guidance for using git branches / PR workflow

7. **Full-text search**: SQLite FTS5 or simple LIKE queries?
   - Start with LIKE queries (simpler)
   - Add FTS5 in phase 2

8. **Transactions**: When do we stage temp files vs. streaming writes?
   - Issue creation (issue + labels + event) should commit atomically
   - Dependency changes must include cycle check + append in same rewrite
   - Bulk operations may require batch temp rewrites
...
4. **Portability**: Store file can be moved between machines, checked into git
