...
- [x] Restructure the project into the four-crate workspace (`beads`, `beads-core`, `beads-merge-driver`, `beads-tracing`) with shared dependencies defined in the root manifest.
- [x] Scaffold the `beads-core` crate modules (`model`, `error`, `repo`, `db`, `log`) and expose the initial public API for repo discovery and initialization.
- [x] Implement the `beads` CLI crate with tracing initialization and command routing for `init`, `create`, `list`, `update`, and `sync`.
- [x] Provide the `beads-merge-driver` binary implementing the line-union merge strategy for `events.jsonl`.
- [x] Add the `beads-tracing` utility crate for standardized tracing setup across binaries.
- [x] Establish SQLite schema management and monotonic ULID generation in `beads-core` to support offline-first workflows.
- [x] Implement local log append and cache update paths for create/update issue flows.
- [x] Wire incremental and full sync paths that reconcile the log with the cache per spec Section 4.2.
- [ ] Cover the new functionality with unit tests targeting core log/DB reconciliation logic.
