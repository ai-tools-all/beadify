use anyhow::Result;
use beads_core::{
    add_issue_dependency, get_dependents, get_issue, get_open_dependencies,
    remove_issue_dependency, repo::BeadsRepo,
};
...
pub fn show(repo: BeadsRepo, issue_id: &str) -> Result<()> {
    ...
    println!(
        "Dependencies for {} {} - {}",
        indicator, issue.id, issue.title
    );
    println!();
    ...
    if !blockers.is_empty() {
        ...
        for blocker_id in blockers {
            if let Ok(Some(blocker)) = get_issue(&repo, &blocker_id) {
                ...
                println!(
                    "  ↳ {} {} p{} - {}",
                    blocker_indicator, blocker_id, blocker.priority, blocker.title
                );
            } else {
    ...
    if !dependents.is_empty() {
        ...
        for dependent_id in dependents {
            if let Ok(Some(dependent)) = get_issue(&repo, &dependent_id) {
                ...
                println!(
                    "  ↳ {} {} p{} - {}",
                    dependent_indicator, dependent_id, dependent.priority, dependent.title
                );
            } else {
